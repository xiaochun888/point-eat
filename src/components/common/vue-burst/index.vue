<template>
  <div class="vue-burst">
    <div class="particles">
      <div :style="wrapperStyle" class="particles-wrapper">
        <div :style="contentStyle" ref="content" @click="disintegrate" v-if="$slots.default" class="particles-content" :class="cls">
          <slot></slot>
        </div>
      </div>
      <canvas :style="canvasStyle" ref="canvas" class="particles-canvas"></canvas>
    </div>
  </div>
</template>
<script>
...
/**
 * https://vuejsexamples.com/a-bursting-particles-effects-buttons-component/
 */
// useful funcs
const is = {
  arr: function(a) {
...
export default {
  ...
  data() {
    return {
      ...
      particles: [],
      frame: null,
      contentVisible: !this.visible, // just a flag
  ...
  methods: {
    ...
    disintegrate(options) {
      if (!this.isAnimating && this.contentVisible) {
        // sync animating
    ...
    setup(options) {
      ...
      if (this.mergedOps.duration) {
        this.rect = this.$refs['content'].getBoundingClientRect();
    ...
    addTransforms(value) {
      ...
      this.contentTransform = translateProperty + '(' + -translateValue + '%)';
    },
  ...
  mounted() {
    if (this.$slots.default) {
      this.init();
      if(!this.mergedOps.color) {
        this.$set(
          this.mergedOps,
          'color',
          getCSSValue(this.$refs['content'], 'background-color')
        );
      };
